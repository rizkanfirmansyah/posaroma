...
<script language="javascript">
	...
	function hapus(id,line) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "main.php?menu=app&act=<?php echo obraxabrix('cashier_box') ?>&mxKz=xm8r389xemx23xb2378e23&xndf="+id+"&line="+line+" ";
		}
	}
	...
	function print() {
		...
		window.open('app/cashier_box_print.php?ref='+ref, 'Invoice Print','825','450','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
	
	function subdetail_item(ref, line) {
		window.open("app/cashier_box_subdetail.php?ref="+ref+"&line="+line,"Find","width=1000,height=620,left=50,top=10,toolbar=0,status=0,scroll=1,scrollbars=no");
	}
	...
	function submitForm(tipe)
    {
    	...
    	if(tipe == 'print') {
			...
			$("#cashier_box").attr('action', 'app/cashier_box_print.php')
			   .attr('target', '_BLANK');
			$("#cashier_box").submit();
    ...
    function itemhistory() {
		...
		window.open('app/cashier_box_item_history.php?client_code='+client_code, 'Item History','825','950','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
	
	function chequehistory() {
		var client_code = document.getElementById('client_code').value;
		
		window.open('app/cashier_box_cheque_history.php?client_code='+client_code, 'Cheque History','825','2000','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
</script>
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$ref2 = notran(date('y-m-d'), 'frmcashier_box_pos', '', '', ''); 
					
				include("app/exec/cashier_box_insert.php"); 
				...
				if($xndf != "") {
					...
					if ($delete == "xm8r389xemx23xb2378e23" && $post != "Save Detail") {
						...
						$delete2=new delete;
						$delete2->delete_cashier_box_detail($_REQUEST['xndf'], $_REQUEST['line']);
					}
					
					$sql=$select->list_cashier_box_get_detail($xndf);
				...
				if ($ref != "") {
					$sql=$select->list_cashier_box($ref);
					$row_cashier=$sql->fetch(PDO::FETCH_OBJ);	
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="cashier_box" id="cashier_box" enctype="multipart/form-data" onSubmit="return cekinput('ref,date,location_id,client_code');" >
            	...
            	if ($ref=='') {		
					...
					include_once("cashier_box_detail_input.php");
					
					if($xndf != "") {
						include("cashier_box_get_detail.php");
					} /*else {
						include("cashier_box_detail.php");		
					}*/
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmcashier_box_pos')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmcashier_box_pos')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Save" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmcashier_box_pos')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . obraxabrix(cashier_box_view) ?>'" />
						...
						&nbsp;&nbsp;&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="Form Baru" onclick="self.location='<?php echo $nama_folder . obraxabrix(cashier_box) ?>'" />
