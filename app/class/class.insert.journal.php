...
class insert_journal{
		...
		try {
			...
			for ($i=0; $i<=$jmldata; $i++) {
				...
				if ( !empty($account_code) && ($debit_amount <> 0 || $credit_amount <> 0) ) { 		
					...
					if($debit_amount  < 0) { //credit
						$debit_amount = $debit_amount * -1;
						$sqlstr="insert into jrn(ivino, ivitpe, ividte, acccde, dcr, dbtamt, crdamt, curcde, excrte, keycde, lne, uid, dlu, sysdte) values('$ivino', 'direct_payment', '$dte', '$account_code', '$mmo', '0', '$debit_amount', 'idr', '0', '$keycode2', '$j', '$uid', '$dlu', '$dlu' )";
						$sql=$dbpdo->prepare($sqlstr);
						$sql->execute();			
					}
			...
			if($amount < 0) { 
				##debet
				$amount = $amount * -1;
				$sqlstr="insert into jrn(ivino, ivitpe, ividte, acccde, dcr, dbtamt, crdamt, curcde, excrte, keycde, lne, uid, dlu, sysdte) values('$ivino', 'direct_payment', '$dte', '$account_code', '$memo', '$amount', '0', 'idr', '0', '$keycode', '0', '$uid', '$dlu', '$dlu' )";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			}
	...
	function journal_delivery_order($ref){
		...
		try {
			...
			for ($i=0; $i<=$jmldata; $i++) {
				...
				$sqlitm=$dbpdo->prepare($sqlitm);
				$sqlitm->execute();
				$dataitm			=	$sqlitm->fetch(PDO::FETCH_OBJ);
				//------------------/\
				...
				$resultitmtype=$dbpdo->prepare($sqlitmtype);
				$resultitmtype->execute();
				$dataitmtype			=	$resultitmtype->fetch(PDO::FETCH_OBJ);
				...
				$resultitmgr=$dbpdo->prepare($sqlitmgrp);
				$resultitmgr->execute();
				$dataitmgrp	= $resultitmgr->fetch(PDO::FETCH_OBJ);
	...
	function journal_cash_invoice($ref){
		...
		try {
			...
			$resultinv=$dbpdo->prepare($sqlinv);
			$resultinv->execute();
			$datainv	= $resultinv->fetch(PDO::FETCH_OBJ);
					
			$client_code	=	$datainv->client_code;
			
			//-------get client type
			$sqlclient		=	"select a.client_type from client a where a.syscode='$client_code' ";
			$resultclient=$dbpdo->prepare($sqlclient);
			$resultclient->execute();
			$dataclient	= $resultclient->fetch(PDO::FETCH_OBJ);
			//--------------------/\
			...
			$resultclntpe=$dbpdo->prepare($sqlclntpe);
			$resultclntpe->execute();
			$dataclntpe	= $resultclntpe->fetch(PDO::FETCH_OBJ);
			...
			for ($i=0; $i<=$jmldata; $i++) {
				...
				$resultitm=$dbpdo->prepare($sqlitm);
				$resultitm->execute();
				$dataitm	= $resultitm->fetch(PDO::FETCH_OBJ);
				//------------------/\
			...
			$resulttax=$dbpdo->prepare($sqltax);
			$resulttax->execute();
			$datatax	= $resulttax->fetch(PDO::FETCH_OBJ);
	...
	function journal_good_receipt($ref){
		...
		try {
			...
			for ($i=0; $i<=$jmldata; $i++) {
				...
				$resultpo=$dbpdo->prepare($sqlpo);
				$resultpo->execute();
				$datapo	= $resultpo->fetch(PDO::FETCH_OBJ);
				...
				$dataitm	=	mysql_fetch_object($resultitm);
				//------------------/\
	...
	function journal_purchase_invoice($ref){
		...
		try {
			...
			$datainv		=	mysql_fetch_object($resultinv);		
			$client_code	=	$datainv->client_code;
			*/
			
			//-------get vendor type
			$sqlvendor		=	"select a.vendor_type from vendor a where a.syscode='$vendor_code' ";
			$resultvendor=$dbpdo->prepare($sqlvendor);
			$resultvendor->execute();
			$datavendor	= $resultvendor->fetch(PDO::FETCH_OBJ);
			//--------------------/\
			...
			$resultvdrtpe=$dbpdo->prepare($sqlvdrtpe);
			$resultvdrtpe->execute();
			$datavdrtpe	= $resultvdrtpe->fetch(PDO::FETCH_OBJ);
	...
	function journal_payment($ref){
		...
		try {
			...
			$resultvdr=$dbpdo->prepare($vdrquery);
			$resultvdr->execute();
			$datavdrtpe	= $resultvdr->fetch(PDO::FETCH_OBJ);
			...
			if($total > 0 && $account_code <> "" ) { 
				##debit
				$sqlstr="insert into jrn(ivino, ivitpe, ividte, acccde, dcr, dbtamt, crdamt, curcde, excrte, keycde, lne, uid, dlu, sysdte) values('$ivino', 'payment', '$dte', '$account_code', '$memo', '$total','0', 'idr', '0', '$keycode', '0', '$uid', '$dlu', '$dlu' )";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			} 
			if($total < 0) { 
				##credit
				$total = $total * -1;
				$sqlstr="insert into jrn(ivino, ivitpe, ividte, acccde, dcr, dbtamt, crdamt, curcde, excrte, keycde, lne, uid, dlu, sysdte) values('$ivino', 'payment', '$dte', '$account_code', '$memo', '0','$total', 'idr', '0', '$keycode', '0', '$uid', '$dlu', '$dlu' )";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			}
			...
			if($round_amount > 0 && $round_amount_account <> "" ) { 
				##debit
				$sqlstr="insert into jrn(ivino, ivitpe, ividte, acccde, dcr, dbtamt, crdamt, curcde, excrte, keycde, lne, uid, dlu, sysdte) values('$ivino', 'payment', '$dte', '$round_amount_account', '$memo', '0','$round_amount', 'idr', '0', '$keycode', '1', '$uid', '$dlu', '$dlu' )";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			} 
			if($round_amount < 0) { 
				...
				$sqlstr="insert into jrn(ivino, ivitpe, ividte, acccde, dcr, dbtamt, crdamt, curcde, excrte, keycde, lne, uid, dlu, sysdte) values('$ivino', 'payment', '$dte', '$round_amount_account', '$memo', '$round_amount','0', 'idr', '0', '$keycode', '1', '$uid', '$dlu', '$dlu' )";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			}
			...
			if($bank_charge > 0 && $bank_charge_account <> "" ) { 
				##debit
				$sqlstr="insert into jrn(ivino, ivitpe, ividte, acccde, dcr, dbtamt, crdamt, curcde, excrte, keycde, lne, uid, dlu, sysdte) values('$ivino', 'payment', '$dte', '$bank_charge_account', '$memo', '0','$bank_charge', 'idr', '0', '$keycode', '2', '$uid', '$dlu', '$dlu' )";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			} 
			if($bank_charge < 0) { 
				...
				$sqlstr="insert into jrn(ivino, ivitpe, ividte, acccde, dcr, dbtamt, crdamt, curcde, excrte, keycde, lne, uid, dlu, sysdte) values('$ivino', 'payment', '$dte', '$bank_charge_account', '$memo', '$bank_charge','0', 'idr', '0', '$keycode', '2', '$uid', '$dlu', '$dlu' )";
				$sql=$dbpdo->prepare($sqlstr);
				$sql->execute();
			}
	...
	function journal_receipt($ref){
		...
		try {
			...
			$resultcln=$dbpdo->prepare($clnquery);
			$resultcln->execute();
			$dataclntpe	= $resultcln->fetch(PDO::FETCH_OBJ);
	...
	//-----journal modal pos petty cash receipt
	function journal_pos_modal_cash_receipt($ref, $account_code, $account_code_det, $memo1, $memo_det){
		$dbpdo = DB::create();
		...
		try {
			...
			$ivino			=	$ref;
			$dte			=	date('Y-m-d');
			$account_code	=	$account_code; //$_POST['account_code'];
			$memo			=	$memo1; //$_POST['memo'];
			//$amount			=	numberreplace($_POST['amount']);
			$uid			=	$_SESSION["loginname"];
			$dlu			=	date("Y-m-d H:i:s");
			
			$amount	= 0;
			$jmldata = 0; //(empty($_POST['jmldata'])) ? 0 : $_POST['jmldata'];
			for ($i=0; $i<=$jmldata; $i++) {
				
				$acccde 			= $account_code_det; //$_POST[account_code_.$i];
				$mmo 				= $memo_det; //$_POST[memo_.$i];
				$crdamt				= numberreplace($_POST['modal']);
