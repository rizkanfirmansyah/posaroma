...
?>
		<script>
			window.location = 'main.php?menu=app&act=<?php echo obraxabrix(purchase_inv) ?>&xndf=<?php echo $ref ?>'			
		</script>
...
if ($post == "Save" ){
	...
	$ref=notran($date, 'frmpurchase_inv_pos', '', '', ''); //---get no ref
		
	$hs=$insert->insert_purchase_inv($ref, $xndf, $ref_tmp);
	
	notran($date, 'frmpurchase_inv_pos', 1, '', '') ; //----eksekusi ref
...
?>
		...
		<script>
			window.location = 'main.php?menu=app&act=<?php echo obraxabrix(purchase_inv) ?>&search=<?php echo $ref ?>';			
		</script>
