<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "main.php?menu=app&act=<?php echo obraxabrix('item_view') ?>&mxKz=xm8r389xemx23xb2378e23&id="+id+" ";
		}
	}
</script>
...
<script type="text/javascript">
	function excel_export() {
		...
		document.location.href = '<?php echo $__folder ?>app/item_xls.php?all='+all+'&code='+code+'&name='+name+'&item_group_id='+item_group_id;
	}
</script>
...
<div class="page-content">		
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<form class="form-horizontal" role="form" action="" method="post" name="item_view" id="item_view" class="form-horizontal" enctype="multipart/form-data" >
            	
            	<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Kode Barang</label>
					<div class="col-sm-3">
						<input type="text" id="code" name="code" class="form-control" value="<?php echo $code ?>">
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<input type="text" id="old_code" name="old_code" class="form-control" value="<?php echo $old_code ?>">
					</div>
				</div>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_item($_REQUEST['id']);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <!--<meta http-equiv="Refresh" content="0;url=main.php?menu=app&act=<?php echo obraxabrix('item_view') ?>" />-->
			<?php
							...
							<tbody>
                                <?php
									...
									$i++;
										...
										$item_qty = "";
										$item_price = "";
										//$sqlitem = $selectview->list_qty_price_last($row_item->syscode, $row_item->uom_code_sales);
										$sqlitem = $select->list_set_item_price_last("", $row_item->syscode);
										...
										//get harga beli
										$sqlcost = $selectview->list_purchase_invoice_last_discount($row_item->syscode, $row_item->uom_code_purchase);
										$datacost = $sqlcost->fetch(PDO::FETCH_OBJ);
										$hargadiscount = ($datacost->unit_cost * $datacost->discount1)/100;
										$hargabeli = $datacost->unit_cost-$hargadiscount;
										$hargabeli = number_format($hargabeli,2,".",",");
										
										
										
										
										//get stok akhir
										$item_group_id2 = $row_item->item_group_id;
										$date_from = date("Y-m-d");
										$date_to = date("Y-m-d");
										$sql1 = $selectview->rpt_stock($row_item->syscode, "", $row_item->uom_code_purchase, "", $item_group_id2, "", $date_from, $date_to);	
										$row_bincard=$sql1->fetch(PDO::FETCH_OBJ);
										
													
										$sql3 = $selectview->rpt_bincard_openblc_item($row_item->syscode, $row_item->uom_code_purchase, "", $date_from, "");
										$data_opnblc = $sql3->fetch(PDO::FETCH_OBJ);
										$opnblc = $data_opnblc->opnblc;
							            
							            $total_qty = 0;
							            $total_qty = $opnblc + $row_bincard->qty;
							            
							            
							            ##cek stok opname (jika ada maka query yg dipakai query ini)
										$sql1=$selectview->rpt_stock_stock_opname_limit1($row_item->syscode, "", $row_item->uom_code_purchase, "", $item_group_id2, "", $date_from, $date_to, "stockopname");
										$data1=$sql1->fetch(PDO::FETCH_OBJ);
										$invoice_type=$data1->invoice_type;
										if($invoice_type == "stockopname") {
											$sql2=$selectview->rpt_check_stock_opname($row_item->syscode, "", $row_item->uom_code_purchase, "", $item_group_id2, "", $date_from, $date_to);
											...
											$sql_so = $selectview->rpt_stock_non_so($row_item->syscode, "", $row_item->uom_code_purchase, "", $item_group_id2, "", $date_stok_opname, $date_to);	
								...
								?>
                                        ...
                                        <tr>
					                    	...
					                    	<?php if($_SESSION["adm"] == 1) { ?>
					                    		<td align="right"><?php echo $hargabeli ?></td>
					                    	<?php } ?>
											...
											<td>
                                                ...
                                                <?php //if (allowupd('frmitem_pos')==1) { ?>
    												<a href="main.php?menu=app&act=<?php echo obraxabrix('item') ?>&search=<?php echo $row_item->syscode ?>" class="tooltip-success" data-rel="tooltip" title="Detail">
    													<span class="green">
