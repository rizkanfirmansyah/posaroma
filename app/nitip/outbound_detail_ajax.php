...
switch ($pilih){
	case "getdata":
		...
		$no = $kodex;
		$sqlstr 	= "select syscode, uom_code_stock uom_code, name from item where (code='$kode' or old_code='$kode') limit 1";
...
?>		
			...
			<td align="left">
				<input type="text" id="item_code2" name="item_code2" style="font-size: 11px; width: 150px" class="form-control" onKeyPress="return focusNext('submit_det',event)" onchange="loadHTMLPost3('app/outbound_detail_ajax.php','item_ajax','getdata','item_code2','location_id2')" value="<?php echo $kode ?>" >
			</td>
...
<?php
	...
	case "getdata2":
		...
		$no = $kodex;
		$sqlstr = "select code, syscode, uom_code_stock uom_code, name from item where syscode='$kode' limit 1";
		$sql=$dbpdo->prepare($sqlstr);
		$sql->execute();
		$data 	= $sql->fetch(PDO::FETCH_OBJ); 
