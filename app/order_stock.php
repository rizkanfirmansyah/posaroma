...
<script language="javascript">
	...
	function hapus(id,line) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "main.php?menu=app&act=<?php echo obraxabrix('order_stock') ?>&mxKz=xm8r389xemx23xb2378e23&xndf="+id+"&line="+line+" ";
		}
	}
	...
	function print() {
		...
		window.open('app/order_stock_print.php?ref='+ref, 'Invoice Print','825','450','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
	
	function subdetail_item(ref, line) {
		window.open("app/order_stock_subdetail.php?ref="+ref+"&line="+line,"Find","width=1000,height=620,left=50,top=10,toolbar=0,status=0,scroll=1,scrollbars=no");
	}
	...
	function submitForm(tipe)
    {
    	...
    	if(tipe == 'print') {
			...
			$("#order_stock").attr('action', 'app/order_stock_print.php')
			   .attr('target', '_BLANK');
			$("#order_stock").submit();
    ...
    function itemhistory() {
		...
		window.open('app/order_stock_item_history.php?client_code='+client_code, 'Item History','825','950','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
	
	function chequehistory() {
		var client_code = document.getElementById('client_code').value;
		
		window.open('app/order_stock_cheque_history.php?client_code='+client_code, 'Cheque History','825','2000','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
</script>
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$ref2 = notran(date('y-m-d'), 'frmorder_stock', '', '', ''); 
					
				include("app/exec/order_stock_insert.php"); 
				...
				if ($ref != "") {
					$sql=$select->list_order_stock($ref);
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="order_stock" id="order_stock" enctype="multipart/form-data" onSubmit="return cekinput('ref,date,location_id');" >
            	...
            	<div class="row">
					<div class="col-sm-6">
						<div class="row">
			            	...
			            	<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php if($lng==1) { echo 'Ref. No.'; } else { echo 'No. Order'; } ?> *)</label>
								<div class="col-sm-4">
            	...
            	if ($ref=='') {		
					...
					include_once("order_stock_detail_input.php");
					
				} else {
					include("order_stock_detail.php");		
				} 
				?>
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
                        ...
                        <?php if (allowupd('frmorder_stock')==1) { ?>
                            <?php if($ref!='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Update" />
    						<?php } ?>
                        <?php } ?>
						
                        <?php if (allowadd('frmorder_stock')==1) { ?>
    						<?php if($ref=='') { ?>
    							<input type="submit" name="submit" id="submit" class='btn btn-primary' value="Save" />
    						<?php } ?>
                        <?php } ?>
                        
                        <?php if (allowdel('frmorder_stock')==1) { ?>
                            &nbsp;
						...
						&nbsp;
						<input type="button" name="submit" id="submit" class="btn btn-success" value="List Data" onclick="self.location='<?php echo $nama_folder . obraxabrix(order_stock_view) ?>'" />
