<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "main.php?menu=app&act=<?php echo obraxabrix('outbound_view') ?>&mxKz=xm8r389xemx23xb2378e23&id="+id+" ";
		}
	}
</script>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_outbound($_REQUEST['id']);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=main.php?menu=app&act=<?php echo obraxabrix('outbound_view') ?>" />
			<?php
							...
							<tbody>
                                <?php			
									...
									while($row_outbound=$sql->fetch(PDO::FETCH_OBJ)){
			                                ...
			                                /*$item_name = "";
			                                $sql2=$select->list_outbound_detail($row_outbound->ref);
			                                while($row_outbound_det=$sql2->fetch(PDO::FETCH_OBJ)) {
			                                    if($item_name == "") {
			                                        $item_name =  $row_outbound_det->item_name; 
			                                    } else {
			                                        $item_name =  $item_name . '<br>' . $row_outbound_det->item_name; 
			                                    }
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <?php if (allowupd('frmoutbound_pos')==1) { ?>
    												<a href="main.php?menu=app&act=<?php echo obraxabrix('outbound') ?>&search=<?php echo $row_outbound->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmoutbound_pos')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_outbound->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
