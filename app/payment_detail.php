...
<?php } 
		...
		$sql=$select->list_payment_detail($ref);
 ...
 <table id="simple-table" class="table table-striped table-bordered table-hover" />
	...
	<tbody>
		<?php 
			...
			$amount_due = number_format($row_payment_detail->amount_due, 0, '.', ',');
			...
			$invoice_due_date = date("d-m-Y", strtotime($row_payment_detail->invoice_due_date));
		...
		?>								
			<input type="hidden" id="old_invoice_no_<?php echo $no ?>" name="old_invoice_no_<?php echo $no ?>" value="<?php echo $row_payment_detail->invoice_no; ?>" >
			<input type="hidden" id="old_line_<?php echo $no ?>" name="old_line_<?php echo $no ?>" value="<?php echo $row_payment_detail->line; ?>" >	
			...
			<tr style="background-color:ffffff;"> 
				...
				<td align="right" id="discount<?php echo $no ?>">
					<input type="text" id="discount_<?php echo $no; ?>" name="discount_<?php echo $no; ?>" style="text-align: right; width: 90px" class="form-control" onkeyup="formatangka('discount_<?php echo $no; ?>'), sub_totals('<?php echo $jmldata ?>')" readonly value="<?php echo $discount ?>" >
				</td>
			...
			<?php 
  	        	if ($ref != '') {
					include("payment_giro_update.php");
				}
  	        ?>
