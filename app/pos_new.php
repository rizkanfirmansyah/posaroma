...
<body>
        ...
        <div class="product">
            ...
            <div class="baris">
                <div class="col-8" id="zero-space-footer">
                    <input autofocus onkeyup="myFunction(event)" type="text" style="height: 100%; width:100%; background-color: transparent; font-size: 36px; color:#fff;" id="InputanItem">
                </div>
            ...
            <div class="modal-content">
                ...
                <form action="#" id="inputPayment">
                    <div class="modal-body" style="padding:20px;">
                        ...
                        <input type="text" class="form-control payments" placeholder="Value Ovo" id="ovoPayment" value="0" autofocus name="ovo">&nbsp;
                        <label for="debit">Debit</label>
                        <input type="text" class="form-control payments" placeholder="Value Debit" value="0" autofocus id="debitPayment" name="debit">&nbsp;
                        <label for="cash">Cash</label>
                        <input type="text" class="form-control payments" placeholder="Value Cash" value="0" autofocus id="cashPayment" name="cash">&nbsp;
    ...
    <!-- Modal Request Void-->
    <div class="modal fade" id="voidRequest" tabindex="-1" role="dialog" aria-labelledby="voidRequestTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form action="#" method="post">
                    <div class="modal-header">
                        <h5 class="modal-title" id="voidRequestTitle">REQUEST VOID</h5>
                        <!-- <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button> -->
                    </div>
                    <div class="modal-body text-center" style="padding:20px;">
                        <label for="void">Kode Item</label>
                        <input type="text" name="kode" class="form-control" placeholder="Masukkan kode Item" autofocus id="codeRequestVoid">
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary">Request <i class="fas fa-sign-out-alt"></i></button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <!-- Modal Void-->
    <div class="modal fade" id="voidModal" tabindex="-1" role="dialog" aria-labelledby="voidModalTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form action="#" method="post">
                    <div class="modal-header">
                        <h5 class="modal-title" id="voidModalTitle">VOID</h5>
                        <!-- <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button> -->
                    </div>
                    <div class="modal-body text-center" style="padding:20px;">
                        <table class="table table-hover" style="color:blue;" border="1">
                            <thead style="background-color: #D1d1d1; color:blue;font-weight: 700;">
                                <tr>
                                    <td>QTY</td>
                                    <td>Nama Barang</td>
                            ...
                            <tbody id="myVoid"></tbody>
                        </table>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-danger">Void <i class="fas fa-times"></i></button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    $(document).on('keyup keydown', function(e) {
        ...
        <?php if (!empty($_GET['ref'])) : ?>
            if (key == 'PrintScreen') {
                window.open('new/print.php?ref=<?= $_GET['ref'] ?>', '_blank')
            }
        <?php endif; ?>
        if (key == 'Escape') {
            $('#InputanItem').focus()
            $('#voidRequest').modal('hide')
    ...
    $(document).ready(function() {
        ...
        $('#voidRequest').on('submit', 'form', function(e) {
            e.preventDefault()
            $.ajax({
                url: 'new/request_void.php',
                data: {
                    code: $('#voidRequest input').val(),
                    ref: '<?= $_GET['ref'] ?>',
                },
                type: 'POST',
                success: res => {
        ...
        $('#voidModal').on('submit', 'form', function(e) {
            e.preventDefault()
            $.ajax({
                url: 'new/void.php',
                type: 'POST',
        ...
        $('#voidModal').on('click', '.voidDelete', function() {
            $.ajax({
                url: 'new/delete_request_void.php',
                type: 'POST',
                data: {
                    ref: $(this).data('ref'),
                    code: $(this).data('code')
    ...
    async function myFunction(event) {
        ...
        <?php if (empty($_GET['ref'])) : ?>
            ...
            if (x == 'F4') {
                ...
                $('#ovoPayment').focus()
                $('#discountPayment').val(rupiah($('#disc').val()))
            }
        <?php endif; ?>
        if (x == 'F1') {
            event.preventDefault();
        ...
        $.ajax({
            ...
            success: res => {
                ...
                response.invoice.forEach(data => {
                    ...
                    total += Number(data.total)
                    qty += Number(data.qty)
                    disc += Number(data.discount * data.qty)
                })
        ...
        $.ajax({
            ...
            success: res => {
                ...
                response.invoice.forEach(data => {
                    html += `
                        <tr>
                            <td>${checkVoid(data)}</td>
                            <td>${data.old_code}</td>
                            <td style="text-align: left;">${data.item_name}</td>
                            <td style="text-align: right;"> ${rupiah(data.unit_price)}</td>
                            <td style="text-align: right;">${rupiah(data.discount3)}%</td>
                            <td style="text-align: right;">${rupiah(data.discount)}</td>
                            <td style="text-align: right;">${rupiah(data.amount - data.discount)}</td>
                        </tr>
                    `
                    amount += Number(data.amount - data.discount)
                    qty += Number(data.qty)
                    disc += Number(data.discount * data.qty)
                })

                $('#totalHarga').html(rupiah(amount) + ',-')
                $('#totalQty').html(qty)
                $('#myList').html(html)
                $('#disc').val(disc)
            },
    ...
    function voidModal(params) {
        $.ajax({
            url: 'new/get_void.php',
            type: 'POST',
            data: {
                code: params,
            },
            success: res => {
                ...
                response.forEach(data => {
                    if (data.qty > 0) {
                        ...
                        <td>${data.qty}</td>
                        <td style="text-align: left;">${data.item_name}</td>
                        <td style="text-align: center;"><a class="voidDelete" href="#" data-code="${data.item_code}" data-ref="${data.ref}">X</a></td>
                        </tr>
                        `
                    }
                })
