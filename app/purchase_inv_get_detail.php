	...
	<tbody>
		<?php 
			...
			while($row_purchase_inv_detail=$sql->fetch(PDO::FETCH_OBJ)) { 
				...
				$total = $total + $row_purchase_inv_detail->amount;
				$total2 = $total; //number_format($total, 0, ".", ",");
				
				//member only
				/*if($non_discount == 0) {
					$totalcek = $totalcek + $row_purchase_inv_detail->amount;
				}*/
		...
		?>								
				...
				<!--<input type="hidden" id="discount_<?php echo $no; ?>" name="discount_<?php echo $no; ?>" readonly="" style="text-align: right;" class="form-control" onkeyup="formatangka('discount_<?php echo $no; ?>'), detailvalue('<?php echo $no ?>', '<?php echo $jmldata ?>')" value="<?php echo $discount_det ?>" >-->
				...
				<tr style="background-color:ffffff;" id="item_ajax2_<?php echo $no; ?>" > 
					...
					<td>				
						<input type="text" id="item_code2_<?php echo $no ?>" name="item_code2_<?php echo $no ?>" class="form-control" readonly="" style="width: 80px" value="<?php echo $row_purchase_inv_detail->code; ?>" >
						
					</td>
					
					<td>				
						<input type="text" id="item_name_<?php echo $no ?>" name="item_name_<?php echo $no ?>" class="form-control" readonly="" style="width: 270px" value="<?php echo $row_purchase_inv_detail->item_name; ?>" >
					...
					*/ ?>
					...
					<td align="center">
						<a href="JavaScript:update('<?php echo $row_purchase_inv_detail->ref ?>','<?php echo $no ?>')"><img src="assets/img/check.png" ></a>
					</td>
					
					<td align="center">
						<!--<input type="checkbox" id="delete_<?php echo $no; ?>" name="delete_<?php echo $no; ?>" class="form-control" value="1" >-->
						<a href="JavaScript:hapus('<?php echo $row_purchase_inv_detail->ref ?>','<?php echo $row_purchase_inv_detail->line ?>')"><img src="assets/img/delete.png" ></a>
					</td>
			...
			<tr>
				...
				<td align="center">
					<input type="text" id="discount" name="discount" style="text-align: right; font-size: 14px; font-weight: bold; width: 60px" class="form-control" onkeyup="formatangka('discount'), sub_total('<?php echo $jmldata ?>')" value="<?php echo number_format($discount2,2,".",",") ?>" >
				</td>
			...
			<tr>
				<td colspan="8" align="right" style="font-size: 14px; font-weight: bold;">Total&nbsp;</td>
				<td align="center" id="total_id">
			...
			<tr>
				...
				<td colspan="2" style="font-weight: bold; font-size: 14px">
					F3 = Bayar <br>
					F2 = Kembali ke Kode Barang
				</td>
				<td colspan="5">&nbsp;</td>
				<td align="right" style="font-size: 14px; font-weight: bold;">Bayar&nbsp;</td>
				<td align="right">
					<input type="text" id="cash_amount" name="cash_amount" autocomplete="off" style="text-align: right; font-size: 14px; font-weight: bold; width: 150px" class="form-control" onkeyup="formatangka('cash_amount'), sub_total('<?php echo $jmldata ?>')" value="<?php echo $cash_amount ?>" >
				</td>
			</tr>
			...
			<tr>
				...
				<td align="right" id="change_amount_id">
					<input type="text" id="change_amount" name="change_amount" readonly="" style="text-align: right; font-size: 14px; font-weight: bold; width: 150px" class="form-control" onkeyup="formatangka('change_amount')" value="<?php echo number_format($change_amount,2,".",",") ?>" >
				</td>
			</tr>
