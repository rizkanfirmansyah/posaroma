...
$sql = $selectview->list_purchase_quick($ref);
...
while($row_purchase_quick_detail=$sql2->fetch(PDO::FETCH_OBJ)) {
	...
	$item_code	=	$row_purchase_quick_detail->old_code;
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Image('../assets/img/logo.jpg', 12, 9, 23, 20, 'jpg', '');
		$this->Ln(1);
	...
	function BasicTable($header,$data)
	{
		...
		foreach($header as $col) {
			...
			if ($i==1) { $this->Cell(25,7,$col,1,0,"C"); }
			if ($i==2) { $this->Cell(112,7,$col,1,0,"C"); }
		...
		{	
			...
			foreach($row as $col) {
				...
				if ($i==1) { $this->Cell(25,6,$col,1,0,"L"); }
				if ($i==2) { $this->Cell(112,6,$col,1,0,"L"); }
