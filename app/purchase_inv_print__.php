...
$name_company = "SAHABAT PUTRA";
if(!empty($dataunit->name)) {
	$name_company = $dataunit->name;
}

$businiss_type = "TOKO RETAIL & GROSIR";
...
while($row_cash_invoice_detail=$sql2->fetch(PDO::FETCH_OBJ)) {
	...
	$discount	=	number_format($row_cash_invoice_detail->discount,"2",",",".");
...
class PDF extends FPDF
{
	...
	function Header()
	{
        ...
        */
		...
		//$this->Cell(1,5,'',0,1,'L',false);
		$this->Cell(0,5,$businiss_type,0,1,'L',false);
		//$this->Ln(1);
		
		//$this->Cell(1,5,'',0,0,'L',false);
		$this->Cell(0,5,'Alamat Kantor :' . $address,0,1,'L',false);
		//$this->Ln(2);
		...
		$this->Line(100, 35, 10, 35);
		//$this->Ln(2);
        //--------/\------
        
		$this->SetFont('arial','U',8);
		$this->Cell(50,5,$ref,0,0,'L',false);
		$this->Cell(40,5,$date,0,1,'R',false);
		//$this->Ln(2);
		
		$this->SetFont('arial','',8);
		$this->Cell(22,5,'Nama Customer',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->Cell(120,5,$client_name,0,1,'L',false);
		//$this->Cell(20,5,'Tanggal : ' . $date,0,1,'L',false);
		//$this->Ln(2);
		
		/*$this->Cell(34,5,'Alamat Pengiriman',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->MultiCell(120,5,$ship_to,0,1,'L',true);*/
		//$this->Ln(2);
		
		/*$this->Cell(34,5,'Alamat Penagihan',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->MultiCell(120,5,$bill_to,0,1,'L',true);*/
		//$this->Ln(2);
	...
	function BasicTable($header,$data)
	{
		...
		foreach($header as $col) {
			...
			if ($i==2) { $this->Cell(40,6,$col,0,0,"C"); }
			if ($i==3) { $this->Cell(18,6,$col,0,0,"C"); }
			if ($i==4) { $this->Cell(15,6,$col,0,0,"C"); }
		...
		{	
			...
			foreach($row as $col) {
				...
				if ($i==0) { 
                    $this->Cell(23,6,$col,0,1,"C"); 
                    //$this->Cell(8,6,$data2[$i][0],1,0,"C");
                }
				if ($i==1) { 
                    $this->Cell(10,6,$col,0,0,"C"); 
                }
				if ($i==2) { $this->Cell(15,6,$col,0,0,"L"); }
				if ($i==3) { $this->Cell(18,6,$col,0,0,"R"); }
				if ($i==4) { $this->Cell(27,6,$col,0,0,"R"); }
				if ($i==5) { $this->Cell(20,6,$col,0,0,"R"); }
				$i++;
			}
			$this->Ln();
		...
		$freight_cost	=	number_format($freight_cost,"2",",",".");
		$discount2		=	number_format($discount2,"2",",",".");
        
        $total_tmp      =   $total + $freight_cost_tmp;
		$total			=	number_format($total_tmp,"2",",",".");
        ...
        $this->Cell(65,5,'Discount',0,0,'R',false);
		$this->Cell(25,5,$discount2,0,1,'R',false);	
		//$this->Cell(144,5,'',0,0,'L',true);	
		$this->Cell(65,5,'Total',0,0,'R',false);
		...
		$this->SetFont('arial','',8);
		//$this->Cell(144,5,'',0,0,'L',true);	
		$this->Cell(6,5,'Bayar',0,0,'R',false);
		...
		//$this->Cell(144,5,'',0,0,'L',true);	
		$this->Cell(65,5,'Kembalian',0,0,'R',false);
