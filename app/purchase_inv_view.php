		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_purchase_inv($_REQUEST['id']);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=main.php?menu=app&act=<?php echo obraxabrix('purchase_inv_view') ?>" />
			<?php
						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
			                        ...
			                        <th><?php if($lng==1) { echo 'Item Name'; } else { echo 'Nama Barang'; } ?></th>						            <th><?php if($lng==1) { echo 'Amount'; } else { echo 'Jumlah'; } ?></th>
			                        <th><?php if($lng==1) { echo 'Amount Tax'; } else { echo 'Jumlah Pajak'; } ?></th>
							...
							<tbody>
                                <?php			
									$sql=$select->list_purchase_inv($kode, $from_date, $to_date, $vendor_code, $all);
									while($row_purchase_inv=$sql->fetch(PDO::FETCH_OBJ)){
			                                ...
			                                while($row_purchase_inv_det=$sql2->fetch(PDO::FETCH_OBJ)) {
			                                    if($item_name == "") {
			                                        $item_name =  $row_purchase_inv_det->item_name . " | QTY: " . $row_purchase_inv_det->qty; 
			                                    } else {
			                                        $item_name =  $item_name . '<br>' . $row_purchase_inv_det->item_name . " | QTY: " . $row_purchase_inv_det->qty; 
			                                    }
			                                ...
			                                if($row_purchase_inv->payment_type == "cash") {
												$payment_type = "Tunai";
											}
											if($row_purchase_inv->payment_type == "credit") {
												$payment_type = "Kredit";
											}
											if($row_purchase_inv->payment_type == "consign") {
								...
								?>
                                        ...
                                        <tr>
											...
											<!--<td><?php echo $row_purchase_inv->memo ?></td>-->
				                            <td>
				                            	<?php //echo $item_name ?>
				                            	...
				                            	<table width="100%" border="1" style="border: 1px solid #93d145">
				                            		...
				                            		<tr>
				                            			...
				                            			<td align="center"><?php echo number_format($row_purchase_inv_det->qty,2,".",",") ?></td>
				                            			<td align="right"><?php echo number_format($row_purchase_inv_det->unit_cost,2,".",",") ?></td>
				                            			<td align="right"><?php 
				                            				
				                            				if($row_purchase_inv_det->discount1 > 0) {
																echo number_format($row_purchase_inv_det->discount1,2,".",",")."%";
															} 
				                            				
				                            				?></td>
				                            			<td align="right"><?php echo number_format($row_purchase_inv_det->amount,2,".",",") ?></td>
				                            		</tr>
				                            		<?php
				                            				$rowsno--;
														}
				                            ...
				                            <td align="right"><?php echo number_format($row_purchase_inv->total,"2",".",",") ?></td>
				                            
											<td><?php echo $row_purchase_inv->dlu ?></td>
                                            <td>
                                            
                                                <?php if (allowupd('frmpurchase_inv_pos')==1) { ?>
    												<a href="main.php?menu=app&act=<?php echo obraxabrix('purchase_inv') ?>&search=<?php echo $row_purchase_inv->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmpurchase_inv_pos')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_purchase_inv->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
