<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "main.php?menu=app&act=<?php echo obraxabrix('purchase_quick_view') ?>&mxKz=xm8r389xemx23xb2378e23&id="+id+" ";
		}
	}
</script>
		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_purchase_quick($_REQUEST['id']);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=main.php?menu=app&act=<?php echo obraxabrix('purchase_quick_view') ?>" />
			<?php
						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th><?php if($lng==1) { echo 'Invoice No'; } else { echo 'No. Nota Manual'; } ?></th>
									<th>Supplier</th>
									<th><?php if($lng==1) { echo 'Memo'; } else { echo 'Catatan'; } ?></th>
							...
							<tbody>
                                <?php			
									$sql=$select->list_purchase_quick($kode, $from_date, $to_date, $vendor_code, $all);
									while($row_purchase_quick=$sql->fetch(PDO::FETCH_OBJ)){
									
			                                $item_name = "";
			                                $sql2=$select->list_purchase_quick_detail($row_purchase_quick->ref);
			                                while($row_purchase_quick_det=$sql2->fetch(PDO::FETCH_OBJ)) {
			                                    if($item_name == "") {
			                                        $item_name =  $row_purchase_quick_det->item_name; 
			                                    } else {
			                                        $item_name =  $item_name . '<br>' . $row_purchase_quick_det->item_name; 
			                                    }
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td>
                                            <td><?php echo $row_purchase_quick->ref ?></td>
						                    <td><?php echo $row_purchase_quick->date ?></td>
						                    <td><?php echo $row_purchase_quick->invoice_no ?></td>
						                    <td><?php echo $row_purchase_quick->vendor_name ?></td>
											<td><?php echo $row_purchase_quick->memo ?></td>
				                            <td><?php echo $item_name ?></td>
				                            <td align="right"><?php echo number_format($row_purchase_quick->total,"0",".",",") ?></td>
											<td><?php echo $row_purchase_quick->dlu ?></td>
                                            <td>
                                            
                                                <?php if (allowupd('frmpurchase_quick')==1) { ?>
    												<a href="main.php?menu=app&act=<?php echo obraxabrix('purchase_quick') ?>&search=<?php echo $row_purchase_quick->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmpurchase_quick')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_purchase_quick->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
