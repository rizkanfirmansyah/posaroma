		...
		<div class="col-xs-12">
            ...
            <?php
				...
				if ($delete == "xm8r389xemx23xb2378e23") {
					...
					$delete2=new delete;
					$delete2->delete_purchase_return($_REQUEST['id']);
			?>
					<div class="alert alert-success">
						<strong>Delete Data successfully</strong>
					</div>
                    
                    <meta http-equiv="Refresh" content="0;url=main.php?menu=app&act=<?php echo obraxabrix('purchase_return_view') ?>" />
			<?php
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form class="form-horizontal" role="form" action="" method="post" name="purchase_return_view" id="purchase_return_view" class="form-horizontal" enctype="multipart/form-data" >
						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
			                        ...
			                        <th><?php if($lng==1) { echo 'Total'; } else { echo 'Jumlah'; } ?></th>						            	<th><?php if($lng==1) { echo 'Date Last Update'; } else { echo 'Terakhir diupdate'; } ?></th>
									<th></th>
							...
							<tbody>
                                <?php			
									$sql=$select->list_purchase_return_quick($kode, $from_date, $to_date, $vendor_code, $all);
									while($row_purchase_return=$sql->fetch(PDO::FETCH_OBJ)){
									
									$status = "";
									if ($row_purchase_return->status == "R") {
										$status = "Released";
									}
									$i++;
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td>
                                            <td><?php echo $row_purchase_return->ref ?></td>
						                    <td><?php echo $row_purchase_return->date ?></td>
						                    <td><?php echo $row_purchase_return->vendor_name ?></td>
											<td>
												<?php //echo $status ?>
												...
												<table width="100%" border="1" style="border: 1px solid #93d145">
				                            		...
				                            		<?php 
				                            			$sql2=$select->list_purchase_return_quick_detail($row_purchase_return->ref); 
				                            			$rowsno=$sql2->rowCount();
				                            			while($row_purchase_return_det=$sql2->fetch(PDO::FETCH_OBJ)) {	
				                            		?>
				                            		<tr>
				                            			<td><?php echo $rowsno; ?>.</td>
				                            			<td><?php echo $row_purchase_return_det->item_name ?></td>
				                            			<td align="center"><?php echo number_format($row_purchase_return_det->qty,2,".",",") ?></td>
				                            			<td align="right"><?php echo number_format($row_purchase_return_det->unit_cost,2,".",",") ?></td>
				                            			<td align="right"><?php 
				                            				
				                            				if($row_purchase_return_det->discount1 > 0) {
																echo number_format($row_purchase_return_det->discount1,2,".",",")."%";
															} 
				                            				
				                            				?></td>
				                            			<td align="right"><?php echo number_format($row_purchase_return_det->amount,2,".",",") ?></td>
				                            		</tr>
				                            		<?php
				                            				$rowsno--;
														}
											...
											<td align="right"><?php echo number_format($row_purchase_return->total,"2",".",",") ?></td>
											<td><?php echo $row_purchase_return->dlu ?></td>
                                            <td>
                                            
                                                <?php if (allowupd('frmpurchase_return_quick_pos')==1) { ?>
    												<a href="main.php?menu=app&act=<?php echo obraxabrix('purchase_return_quick') ?>&search=<?php echo $row_purchase_return->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmpurchase_return_quick_pos')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_purchase_return->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
