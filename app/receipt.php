	...
	function cekinput(fid) {  
	  ...
	  for(i=0; i < arrf.length; i++) {
		if(document.getElementById(arrf[i]).value=='') {       
		  ...
		  if (document.getElementById(arrf[i]).name=='receipt_type') {
			alert('Receipt Type cannot empty!');				
		  }
...
<script>
	...
	function cek_one(id, jmldata){
		...
		if(cek){
			var loan = document.getElementById('loan_'+id).value;
			var installment = document.getElementById('installment_'+id).value.replace(/[^\d-.]/g,"");
			
			var amount_due=document.getElementById('amount_due_'+id).value; 
			if(loan == 1 && installment > 0) {
				amount_due = number_format(installment,0,".",",");	
			} else {
			...
			$('#discount'+id).html('<input type="text" id="discount_'+id+'" name="discount_'+id+'" style="text-align: right; width: 110px" class="form-control" onkeyup="formatangka(\'discount_'+id+'\'), sub_totals('+jmldata+')" value="'+discount+'" >');
			...
			$('#discount'+id).html('<input type="text" id="discount_'+id+'" name="discount_'+id+'" style="text-align: right; width: 110px" class="form-control" onkeyup="formatangka(\'discount_'+id+'\'), sub_totals('+jmldata+')" readonly value="" >');			
...
<script type="text/javascript">
	function receip_history(invoice_no) {
			
		window.open('app/receipt_history.php?invoice_no='+invoice_no, '','825','450','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
</script>
...
<script language="javascript">
	function print() {
		...
		window.open('app/receipt_print.php?ref='+ref, '','825','450','resizable=1,scrollbars=1,status=0,toolbar=0')
	}
</script>
	...
	<div class="row">
		<div class="col-xs-12">
            ...
            <?php 
				...
				$ref2 = notran(date('y-m-d'), 'frmreceipt', '', '', ''); 
					
				include("app/exec/receipt_insert.php"); 
				...
				if ($ref != "") {
					$sql=$select->list_receipt($ref);
					$row_receipt=$sql->fetch(PDO::FETCH_OBJ);	
					
					$ref2 = $row_receipt->ref;	
					$date = date("d-m-Y", strtotime($row_receipt->date));
					
					$receipt_type = $row_receipt->receipt_type;
					
					$credit_card_expired = date("d-m-Y", strtotime($row_receipt->credit_card_expired));
					if($credit_card_expired == "01-01-1970" ) {
						$credit_card_expired = "";
					}
					
					$cheque_date = date("d-m-Y", strtotime($row_receipt->cheque_date));
					if($cheque_date == "01-01-1970" ) {
						$cheque_date = "";
					}
					
					$sub_total = number_format($row_receipt->sub_total, 0, '.', ',');
					$rate = number_format($row_receipt->rate, 0, '.', ',');
					$deposit = number_format($row_receipt->deposit, 0, '.', ',');
					$round_amount = number_format($row_receipt->round_amount, 0, '.', ',');
					$bank_charge = number_format($row_receipt->bank_charge, 0, '.', ',');
					$amount = number_format($row_receipt->amount, 0, '.', ',');
					$total = number_format($row_receipt->total, 0, '.', ',');
				}	
			...
			<!-- PAGE CONTENT BEGINS -->
			<form class="form-horizontal" role="form" action="" method="post" name="receipt" id="receipt" enctype="multipart/form-data" onSubmit="return cekinput('ref,date,receipt_type');" >
            	...
            	<table border="0" width="100%" >
            		...
            		<tr>
                    	<td width="55%">
							...
							<div class="form-group">
		                        ...
		                        <div class="col-sm-3">
		                          <select id="status" name="status" data-placeholder="..." class="chosen-select form-control" style="width: auto">
		                            <?php 
		                            	select_status($row_receipt->status) 
		                            ?>	                            
		                          </select>
								</div>
							</div>
							...
							<div class="form-group">
		                        <label class="col-sm-3 control-label no-padding-right">Receipt Type *)</label>
		                        <div class="col-sm-3">
		                        
		                        <?php if ($ref=='') { ?>
		                          <select id="receipt_type" name="receipt_type" data-placeholder="..." class="chosen-select form-control" style="width:10px" onChange="loadHTMLPost2('app/receipt_type_ajax.php','receipttype','getdata','receipt_type')">
		                            <!--<option value=""></option>-->
		                            <?php 
		                            	select_receipt_type($row_receipt->receipt_type)
		                            ?>	                            
		                          </select>
		                        <?php } else { ?>
		                        	<input type="hidden" id="receipt_type" name="receipt_type" class="form-control" readonly value="<?php echo $row_receipt->receipt_type ?>">
		                        	
									<select id="receipt_type2" name="receipt_type2" data-placeholder="..." class="chosen-select form-control" style="width: auto" disabled >
										<option value=""></option>
											<?php 
												select_receipt_type($row_receipt->receipt_type)
											?>	                            
							...
							<?php if ($ref=='') { ?>
								<div class="form-group" style="margin-left: -8px" id="receipttype">
									<div class="form-group">
									...
									<div class="form-group">
										...
										<div class="col-sm-5">
											<input type="text" id="account_bank" name="account_bank" class="form-control" value="<?php echo $row_payment->account_bank ?>">
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-5">
											<input type="text" id="account_bank_name" name="account_bank_name" class="form-control" value="<?php echo $row_payment->account_bank_name ?>">
										</div>
									</div>
								</div>
							<?php } else { 
							
								include("receipt_type.php");
							} ?>
							...
							<div class="form-group">
		                        ...
		                        <?php if ($ref=='') { ?>
		                          <select id="client_code" name="client_code" data-placeholder="..." class="chosen-select form-control" style="width: auto" onChange="loadHTMLPost2('app/receipt_ajax.php','invoicedetail','getinvoice','client_code')">
		                          	<option value=""></option>
		                            <?php 
		                            	//combo_select_active("client","syscode","name","active","1",$row_receipt->client_code) 
										select_contact3($row_receipt->client_code);
		                            ?>                   
		                          </select>
		                        <?php } else { ?>
		                        	<input type="hidden" id="client_code" name="client_code" class="form-control" readonly value="<?php echo $row_receipt->client_code ?>">
		                        	
		                        	<select id="client_code2" name="client_code2" data-placeholder="..." class="chosen-select form-control" style="width: auto" disabled >
		                          	<option value=""></option>
		                            <?php 
		                            	select_contact3($row_receipt->client_code);
		                            ?>                   
            			...
            			<td>&nbsp;</td>
			             ...
			             <td>
			             	...
			             	<div class="form-group">
		                        ...
		                        <div class="col-sm-3">
		                          <select id="currency_code" name="currency_code" data-placeholder="..." class="chosen-select form-control" style="width: auto">
		                            <option value=""></option>
		                            <?php 
		                            	combo_select_active("currency","syscode","name","active","1",$row_receipt->currency_code)
		                            ?>	                            
		                          </select>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<textarea id="memo" name="memo" class="form-control"><?php echo $row_receipt->memo ?></textarea>
								</div>
							</div>
			             	...
			             	<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" id="uid" name="uid" readonly class="form-control" value="<?php echo $row_receipt->uid ?>">
								</div>
							</div><!-- /.form-group -->
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" id="dlu" name="dlu" readonly class="form-control" value="<?php echo $row_receipt->dlu ?>" >
								</div>
							</div><!-- /.form-group -->
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
						...
						<input type="button" name="submit" class="btn btn-success" value="List" onClick="self.location='main.php?menu=app&act=<?php echo obraxabrix(receipt_view) ?> '">
