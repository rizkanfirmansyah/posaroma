...
$sql = $selectview->list_receipt($ref);
$row_receipt=$sql->fetch(PDO::FETCH_OBJ);

$ref			=	$row_receipt->ref;
$contact_code	=	$row_receipt->contact_code;
	
$contact_name	=	$row_receipt->contact_name;
$address_vdr	=	$row_receipt->address;
$phone_vdr		=	$row_receipt->phone;

$date			=	date("d-m-Y", strtotime($row_receipt->date));

$total 			= 	$row_receipt->total;
$round_amount	= 	$row_receipt->round_amount;
$bank_charge	=	$row_receipt->bank_charge;
...
$sql2 = $selectview->list_receipt_detail($ref);
while($row_receipt_detail=$sql2->fetch(PDO::FETCH_OBJ)) {

	$invoice_type	=	$row_receipt_detail->invoice_type;
	$invoice_no		=	$row_receipt_detail->invoice_no;
	$invoice_date	=	date("d-m-Y", strtotime($row_receipt_detail->invoice_date));
	$amount_due		=	$row_receipt_detail->amount_due;
	$amount 		= 	$row_receipt_detail->amount_paid;
	$sub_total 		= 	$sub_total + $row_receipt_detail->amount_paid;
	
	$total2 		= 	$total2 + $row_receipt_detail->credit_amount;
	...
	if ($invoice_type == "DRC") {
		$invoice_type = "Direct Receipt";
	}
	
	
	$memo	=	$row_receipt_detail->memo;
	$unit_cost	=	number_format($unit_cost,"0",",",".");
	$discount	=	0; //number_format($row_receipt_detail->discount,"0",",",".");
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(145,5,'PENERIMAAN (RECEIPT)',0,0,'L',true);
