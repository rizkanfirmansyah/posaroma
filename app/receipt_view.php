<?php
	...
	if ($delete == "xm8r389xemx23xb2378e23") {
		...
		$delete2=new delete;
		$delete2->delete_receipt($_REQUEST['id']);
?>
...
<script type="text/javascript">
	function hapus(id) {
		if (confirm('Apakah Anda yakin akan menghapus data ini?')) {
			document.location.href = "main.php?menu=app&act=<?php echo obraxabrix('receipt_view') ?>&mxKz=xm8r389xemx23xb2378e23&id="+id+" ";
		}
	}
</script>
...
<div class="page-content">		
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<form class="form-horizontal" role="form" action="" method="post" name="receipt_view" id="receipt_view" class="form-horizontal" enctype="multipart/form-data" >
						...
						<table id="dynamic-table" class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
			                    	...
			                    	<th><?php if($lng==1) { echo 'Receipt Type'; } else { echo 'Tipe Bayar'; } ?></th>
							...
							<tbody>
                                <?php			
									$sql=$select->list_receipt("", $from_date, $to_date, $client_code, $all);
									while($row_receipt=$sql->fetch(PDO::FETCH_OBJ)){
									
										$status = "";
										if ($row_receipt->status == "R") {
											$status = "Released";
										}
										
										$receipt_type = "";
										if ($row_receipt->receipt_type == "cash") {
											$receipt_type = "Cash";
										}
										if ($row_receipt->receipt_type == "giro") {
											$receipt_type = "Bilyet & Giro";
										}
										if ($row_receipt->receipt_type == "cheque") {
											$receipt_type = "Cheque";
										}
										if ($row_receipt->receipt_type == "card") {
											$receipt_type = "Credit Card";
										}
										if ($row_receipt->receipt_type == "transfer") {
								...
								?>
                                        ...
                                        <tr>
                                            <td><?php echo $i ?></td>
                                            <td><?php echo $row_receipt->ref ?></td>
						                    <td><?php echo $row_receipt->date ?></td>
						                    <td><?php echo $row_receipt->client_name ?></td>
											<td><?php echo $status ?></td>
											<td><?php echo $receipt_type ?></td>
											<td align="right"><?php echo number_format($row_receipt->total,0,".",",") ?></td>
											<td><?php echo $row_receipt->dlu ?></td>
											<td>
                                            
                                                <?php if (allowupd('frmreceipt')==1) { ?>
    												<a href="main.php?menu=app&act=<?php echo obraxabrix('receipt') ?>&search=<?php echo $row_receipt->ref ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
    													<span class="green">
                                                ...
                                                <?php if (allowdel('frmreceipt')==1) { ?>    
                                                    &nbsp;
    												<a href="JavaScript:hapus('<?php echo $row_receipt->ref ?>')" class="tooltip-error" data-rel="tooltip" title="Delete">
    													<span class="red">
