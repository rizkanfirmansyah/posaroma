...
while($data_kasir = $sqlcsr->fetch(PDO::FETCH_OBJ)) {
	...
	$sqlx = $selectview->list_pos_print_total('', "", $from_date, $to_date, $shift, $data_kasir->uid, $void);
	$row_total=$sqlx->fetch(PDO::FETCH_OBJ);
	    ...
	    "data": [
	      ';
			...
			while($row_cash_invoice_detail = $sql2->fetch(PDO::FETCH_OBJ)){
				...
				$total_qty_void = 0;
				$sub_total = 0;
				$sql3 = $selectview->close_cash_invoice_detail($from_date, $to_date, $shift, $data_kasir->uid, $receipt_type_pos, $void);
				$rows2 = $sql3->rowCount();
